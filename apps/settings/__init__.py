...
class SettingsFeature:
    ...
    def load_feature_config(self) -> Dict[str, Any]:
        config_path = self.config.get("config_file", "apps/settings/config.json")
        try:
    ...
    def list_all_settings(self, config: Dict[str, Any], prefix: str = "") -> Dict[str, str]:
        ...
        # App enablement settings
        apps = config.get("apps", {})
        for app_name, app_config in apps.items():
            settings[f"apps.{app_name}.enabled"] = str(app_config.get("enabled", False))
            if "config_file" in app_config:
                settings[f"apps.{app_name}.config_file"] = str(app_config["config_file"])
    ...
    async def handle_feature_settings_command(self, message: Message, feature_name: str, args: list, prefix: str) -> bool:
        ...
        apps_config = main_config.get("apps", {})
        
        if feature_name not in apps_config:
            await message.reply(f"❌ App `{feature_name}` not found in configuration!")
            return True
        
        app_config = apps_config[feature_name]
        if not app_config.get("enabled", False):
            await message.reply(f"❌ App `{feature_name}` is not enabled!")
            return True
        
        config_file = app_config.get("config_file")
        if not config_file:
            await message.reply(f"❌ App `{feature_name}` has no config file configured!")
            return True
